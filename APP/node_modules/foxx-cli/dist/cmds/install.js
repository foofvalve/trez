...
const describe = `Installs the given new service at the given ${bold("mount")} path.`;

const args = [["mount", "Mount path of the service"], ["source", `URL or file system path of the service to install. Use ${bold("@")} to pass a zip file from stdin`, '[default: "."]']];
...
})).example("$0 install /hello", 'Install the current working directory as a Foxx service at the URL "/hello"').example("$0 install --dev /hello", "Install the service in development mode").example("$0 install --server http://localhost:8530 /hello", "Use the server on port 8530 instead of the default").example("$0 install --database mydb /hello", 'Use the database "mydb" instead of the default').example("$0 install --server dev /hello", 'Use the "dev" server instead of the default. See the "server" command for details').example("$0 install --no-setup /hello", "Install the service without running the setup script afterwards").example("$0 install /hello demo.zip", 'Install the bundle "demo.zip"').example("$0 install /hello /tmp/bundle.zip", 'Install the bundle located at "/tmp/bundle.zip" (on the local machine)').example("$0 install /hello /tmp/my-service", 'Bundle and install the directory "/tmp/my-service" (on the local machine)').example("$0 install /hello -R /tmp/bundle.zip", 'Install the bundle located at "/tmp/bundle.zip" (on the ArangoDB server)').example("$0 install /hello http://example.com/foxx.zip", 'Download the bundle from "http://example.com/foxx.zip" locally and install it').example("$0 install /hello -R http://example.com/foxx.zip", 'Instruct the ArangoDB server to download the bundle from "http://example.com/foxx.zip" and install it').example("$0 install /hello -d mailer=/mymail -d auth=/myauth", 'Install the service and set its "mailer" and "auth" dependencies').example("cat foxx.zip | $0 install /hello @", "Install the bundle read from stdin");
...
exports.handler = (() => {
  var _ref = _asyncToGenerator(function* (argv) {
    try {
      ...
      const result = yield db.installService(argv.mount, source, _extends({}, opts, {
        setup: argv.setup
      }));
      if (argv.raw) {
        json(result);
