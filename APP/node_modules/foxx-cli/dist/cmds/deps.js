...
const describe = il`Updates or fetches the current dependencies for service at the given ${bold("mount")} path.
...
const args = [["mount", "Mount path of the service"], ["options", `Key-value pairs to apply to the dependencies. Use ${bold("@")} to pass a JSON file from stdin`]];
...
})).example("$0 deps /myfoxx", 'Show the dependencies for the service mounted at "/foxxmail"').example("$0 deps /myfoxx mailer=/foxxmail", 'Sets the "mailer" dependency to the service mounted at "/foxxmail"').example("$0 deps /myfoxx -f mailer=/foxxmail", 'Sets the "mailer" dependency and clears any other dependencies').example('echo \'{"mailer": "/foxxmail"}\' | $0 deps /myfoxx @', "Sets the dependency using JSON data from stdin").example("$0 deps /myfoxx -f", "Clears all configured dependencies");
...
exports.handler = (() => {
  var _ref = _asyncToGenerator(function* (argv) {
    ...
    try {
      ...
      let result;
      if (!options) {
        result = yield db.getServiceDependencies(argv.mount);
      } else if (argv.force) {
        result = yield db.replaceServiceDependencies(argv.mount, options);
      } else {
        result = yield db.updateServiceDependencies(argv.mount, options);
      }
      if (argv.raw) {
        if (argv.minimal) {
        ...
        for (const key of Object.keys(result)) {
          ...
          info(parts.join(" "));
          if (dfn.current === undefined) info(`Mount: ${gray("N/A")}`);else info(`Mount: ${dfn.current}`);
