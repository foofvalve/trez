...
let runTests = (() => {
  var _ref2 = _asyncToGenerator(function* (db, mount, cliReporter) {
    let apiReporter;
    if (cliReporter === "spec") apiReporter = "suite";else if (cliReporter === "json") apiReporter = "default";else if (cliReporter === "list") apiReporter = "default";else if (cliReporter === "min") apiReporter = "default";else apiReporter = cliReporter;

    let result;
    try {
      result = yield db.runServiceTests(mount, { reporter: apiReporter });
    } catch (e) {
      if (e.isArangoError) {
        switch (e.errorNum) {
          ...
          case errors.ERROR_SERVICE_NEEDS_CONFIGURATION:
            fatal(`Service at "${white(mount)}" is missing configuration or dependencies.`);
            break;
    ...
    if (cliReporter === "stream") {
      info(result);
      const lines = result.split("\n");
      process.exit(lines.filter(function (line) {
    ...
    process.exit(result && result.stats && typeof result.stats.failures === "number" ? result.stats.failures : 0);
  });

  return function runTests(_x2, _x3, _x4) {
...
const describe = il`
  ...
  ` + "\n\n" + group(["spec", "Hierarchical specification of nested test cases", "[default]"], ["list", "Simple list of test cases"], ["min", "Just the summary and failures"], ["json", "Single large raw JSON object"], ["tap", "Output suitable for Test-Anything-Protocol consumers"], ["stream", 'Line-delimited JSON stream of "events" beginning with a single "start", followed by "pass" or "fail" for each test and ending with a single "end"'], ["xunit", "Jenkins-compatible xUnit-style XML output"]);
...
})).example("$0 test /hello", 'Runs the tests of a Foxx service at the URL "/hello"').example("$0 test -R json /hello", "Use the json reporter instead of the default").example("$0 test --server http://locahost:8530 /hello", "Use the server on port 8530 instead of the default").example("$0 test --database mydb /hello", 'Use the database "mydb" instead of the default').example("$0 test --server dev /hello", 'Use the "dev" server instead of the default. See the "server" command for details');
...
exports.handler = (() => {
  var _ref = _asyncToGenerator(function* (argv) {
    try {
      ...
      return yield runTests(db, argv.mount, argv.reporter);
    } catch (e) {
      fatal(e);
    }
  });
