...
describe("Foxx service upgraded", () => {
  ...
  for (const c of cases) {
    it(`via ${c.name} should be available`, _asyncToGenerator(function* () {
      yield foxx(`upgrade ${mount} ${c.source(arangoPaths)}`);
  ...
  it("via stdin should be available", _asyncToGenerator(function* () {
    const input = fs.readFileSync(path.resolve(basePath, "minimal-working-service.zip"));
    yield foxx(`upgrade ${mount} @`, false, { input });
    const res = yield db.route(mount).get();
    expect(res.body).to.eql({ hello: "world" });
  }));
  ...
  describe("with a password file", () => {
    ...
    it("should be available", _asyncToGenerator(function* () {
      yield foxx(`upgrade --username ${user} --password-file ${passwordFilePath} ${mount} ${servicePath}`);
      const res = yield db.route(mount).get();
      expect(res.body).to.eql({ hello: "world" });
    }));
  });
  ...
  it("should run its setup script by default", _asyncToGenerator(function* () {
    ...
    yield foxx(`upgrade ${mount} ${path.resolve(basePath, "minimal-working-setup-teardown.zip")}`);
  ...
  it("should run its setup script when enabled", _asyncToGenerator(function* () {
    ...
    yield foxx(`upgrade --setup ${mount} ${path.resolve(basePath, "minimal-working-setup-teardown.zip")}`);
    const info = yield db.collection(col).get();
    expect(info).to.have.property("name", col);
  }));
  ...
  it("should not run its setup script when disabled", _asyncToGenerator(function* () {
    ...
    yield foxx(`upgrade --no-setup ${mount} ${path.resolve(basePath, "minimal-working-setup-teardown.zip")}`);
    try {
      yield db.collection(col).get();
      expect.fail();
    } catch (e) {
  ...
  it("should not run its teardown script by default", _asyncToGenerator(function* () {
    const col = `${mount}_setup_teardown`.replace(/\//, "").replace(/-/g, "_");
    try {
      yield foxx(`upgrade ${mount} ${path.resolve(basePath, "minimal-working-setup-teardown.zip")}`);
      yield foxx(`upgrade ${mount} ${path.resolve(basePath, "minimal-working-service.zip")}`);
      const info = yield db.collection(col).get();
      expect(info).to.have.property("name", col);
    } finally {
      try {
  ...
  it("should run its teardown script when enabled", _asyncToGenerator(function* () {
    const col = `${mount}_setup_teardown`.replace(/\//, "").replace(/-/g, "_");
    yield foxx(`upgrade ${mount} ${path.resolve(basePath, "minimal-working-setup-teardown.zip")}`);
    yield foxx(`upgrade --teardown ${mount} ${path.resolve(basePath, "minimal-working-service.zip")}`);
    try {
      yield db.collection(col).get();
      expect.fail();
    } catch (e) {
  ...
  it("should not run its teardown script when disabled", _asyncToGenerator(function* () {
    ...
    try {
      yield foxx(`upgrade ${mount} ${path.resolve(basePath, "minimal-working-setup-teardown.zip")}`);
      yield foxx(`upgrade --no-teardown ${mount} ${path.resolve(basePath, "minimal-working-service.zip")}`);
      const info = yield db.collection(col).get();
      expect(info).to.have.property("name", col);
    } finally {
      try {
  ...
  it("with configuration should not be set by default", _asyncToGenerator(function* () {
    yield foxx(`upgrade ${mount} ${path.resolve(basePath, "with-configuration.zip")}`);
  ...
  it("with dependencies should have not be set by default", _asyncToGenerator(function* () {
    yield foxx(`upgrade ${mount} ${path.resolve(basePath, "with-dependencies.zip")}`);
  ...
  it("with dependencies should have one dependency set", _asyncToGenerator(function* () {
    yield foxx(`upgrade --dep test1=/test1 ${mount} ${path.resolve(basePath, "with-dependencies.zip")}`);
    const dependencies = yield db.getServiceDependencies(mount, true);
  ...
  it("with dependencies should have two dependencies set", _asyncToGenerator(function* () {
    yield foxx(`upgrade --dep test1=/test1 --dep test2=/test2 ${mount} ${path.resolve(basePath, "with-dependencies.zip")}`);
    const dependencies = yield db.getServiceDependencies(mount, true);
    expect(dependencies).to.have.property("test1", "/test1");
  ...
  it("with dependencies should have one dependency set (short option)", _asyncToGenerator(function* () {
    yield foxx(`upgrade -d test1=/test1 ${mount} ${path.resolve(basePath, "with-dependencies.zip")}`);
    const dependencies = yield db.getServiceDependencies(mount, true);
    expect(dependencies).to.have.property("test1", "/test1");
    expect(dependencies).to.not.have.property("test2");
  }));
  ...
  it("with dependencies should have two dependencies set (short option)", _asyncToGenerator(function* () {
    yield foxx(`upgrade -d test1=/test1 -d test2=/test2 ${mount} ${path.resolve(basePath, "with-dependencies.zip")}`);
    const dependencies = yield db.getServiceDependencies(mount, true);
    expect(dependencies).to.have.property("test1", "/test1");
    expect(dependencies).to.have.property("test2", "/test2");
  }));
  ...
  it("with dependencies should have two dependencies set (mixed options)", _asyncToGenerator(function* () {
    yield foxx(`upgrade --dep test1=/test1 -d test2=/test2 ${mount} ${path.resolve(basePath, "with-dependencies.zip")}`);
    const dependencies = yield db.getServiceDependencies(mount, true);
    expect(dependencies).to.have.property("test1", "/test1");
    expect(dependencies).to.have.property("test2", "/test2");
  }));
  ...
  it("should fail when mount is invalid", _asyncToGenerator(function* () {
    try {
      yield foxx(`upgrade /dev/null ${path.resolve(basePath, "minimal-working-service.zip")}`);
    } catch (e) {
      try {
