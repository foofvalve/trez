...
exports.common = function common(yargs, opts) {
  yargs = yargs.epilog("Copyright (c) 2016-2017 ArangoDB GmbH (https://foxx.arangodb.com)").strict();
...
exports.parseServiceOptions = function parseServiceOptions(argv) {
  ...
  if (argv.remote) {
    if (!argv.source || argv.source === "@") {
      fatal(`Please specify a URL or file path when using ${bold("--remote")}.`);
    }
  } else if (!argv.source) {
  ...
  if (argv.cfg) {
    for (const cfg of argv.cfg) {
      ...
      try {
        ...
        fatal(il`
          Configuration option "${white(name)}" is invalid. Value must be valid JSON: "${white(value)}".
        `);
      }
    }
  }
